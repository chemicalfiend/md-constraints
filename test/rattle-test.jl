...
function RattleTest()
    ...
    rat = RATTLE(bond_lengths, collect(1:(n_atoms÷2)), collect((1 + n_atoms÷2):n_atoms))

    constraint_list = (sh, rat,)
    ...
    for r in 1:length(sh.is)
        ...
        push!(vpset, dot(sys.velocities[sh.is[r]] - sys.velocities[sh.js[r]], vector(sys.coords[sh.is[r]], sys.coords[sh.js[r]], sys.boundary)))
    end
    ...
    if abs(maximum(vpset)) < 1e-15u"nm^2 / ps"
        println("\n SUCCESS!")
    else
        println("\n FAILED!")
    end
